...
const USER_SAFE_DATA = ["firstName", "lastName", "gender", "skills","photoURL"];

userRouter.get("/user/request/received", userAuth, async function (req, res) {
	try {
		const user = req.user;
...
userRouter.get("/user/connections", userAuth, async function (req, res) {
	try {
		...
		res.json({ message: "successfully fetch the connections request", data: user });
